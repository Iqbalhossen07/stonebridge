...
<body class="bg-secondary font-body text-slate-800 antialiased">
    <div class="flex h-screen bg-secondary overflow-hidden">
        ...
        <div class="flex-1 flex flex-col overflow-y-auto relative">
            <div aria-hidden="true" class="absolute inset-0 -z-10 overflow-hidden pointer-events-none"><svg class="absolute top-0 left-0 w-full h-auto text-primary" viewBox="0 0 1440 900" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice"><path d="M0 200C0 200 360 400 720 200C1080 0 1440 200 1440 200V900H0V200Z" fill="url(#p0)" /><path d="M0 150C0 150 360 300 720 150C1080 0 1440 150 1440 150V900H0V150Z" fill="url(#p1)" style="mix-blend-mode: overlay" /><defs><linearGradient id="p0" x1="720" y1="0" x2="720" y2="900" gradientUnits="userSpaceOnUse"><stop stop-color="currentColor" stop-opacity="0.05" /><stop offset="1" stop-color="currentColor" stop-opacity="0" /></linearGradient><linearGradient id="p1" x1="720" y1="0" x2="720" y2="900" gradientUnits="userSpaceOnUse"><stop stop-color="currentColor" stop-opacity="0.04" /><stop offset="1" stop-color="currentColor" stop-opacity="0" /></linearGradient></defs></svg></div>
            
            <header class="bg-white/80 backdrop-blur-sm shadow-soft-1 sticky top-0 z-20">
                <div class="container mx-auto px-6 py-3 flex items-center justify-between"><div class="flex items-center gap-4"><button id="open-menu" class="lg:hidden text-slate-600 p-2 rounded-full hover:bg-slate-100"><i class="fas fa-bars"></i></button><h1 class="font-heading text-xl md:text-2xl font-bold text-primary">Add New Post</h1></div><div class="flex items-center gap-3 md:gap-5"><button class="flex items-center gap-3 group"><img src="https://images.unsplash.com/photo-1570295999919-56ceb5ecca61?q=80&w=800&auto=format&fit=crop" alt="User" class="w-10 h-10 rounded-full object-cover"><div class="hidden sm:block text-left"><p class="font-heading text-sm font-semibold text-primary">David Lee</p><p class="text-xs text-slate-500">Administrator</p></div></button></div></div>
            </header>
                    ...
                    <form action="#" method="POST" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="lg:col-span-2 space-y-6">
                            <div>
                                ...
                                <input type="text" id="post-title" class="block w-full rounded-lg border-slate-300 bg-slate-50 shadow-sm focus:border-primary focus:ring-primary/50" placeholder="Enter post title...">
                            </div>
                            <div>
                                <label for="post-content" class="block text-sm font-medium text-slate-700 mb-1">Post Content</label>
                                <textarea id="post-content" rows="12" class="block w-full rounded-lg border-slate-300 bg-slate-50 shadow-sm focus:border-primary focus:ring-primary/50" placeholder="Write your blog post here..."></textarea>
                            </div>
                        </div>
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <div id="image-upload-box" class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 border-dashed rounded-lg bg-slate-50">
                                    <div class="space-y-1 text-center">
                                        <i class="fas fa-cloud-upload-alt text-4xl text-slate-400"></i>
                                        <div class="flex text-sm text-slate-600">
                                            <label for="file-upload" class="relative cursor-pointer bg-transparent rounded-md font-medium text-primary hover:text-primary-dark focus-within:outline-none">
                             ...
                             <div>
                                <label for="post-category" class="block text-sm font-medium text-slate-700 mb-1">Category</label>
                                <select id="post-category" class="block w-full rounded-lg border-slate-300 bg-slate-50 shadow-sm focus:border-primary focus:ring-primary/50">
                            ...
                            <div>
                                <label for="post-tags" class="block text-sm font-medium text-slate-700 mb-1">Tags</label>
                                <input type="text" id="post-tags" class="block w-full rounded-lg border-slate-300 bg-slate-50 shadow-sm focus:border-primary focus:ring-primary/50" placeholder="e.g., visa, settlement, law">
                            </div>
                            <div class="border-t border-slate-200 pt-6 flex items-center justify-end gap-3">
                                <button type="button" class="bg-slate-200 hover:bg-slate-300 text-slate-800 font-bold py-2 px-4 rounded-lg transition-colors duration-300">Save Draft</button>
                                <button type="submit" class="bg-primary hover:bg-primary-dark text-white font-bold py-2 px-4 rounded-lg transition-colors duration-300">Publish</button>
                            </div>
                        </div>
                    </form>
                </div>
